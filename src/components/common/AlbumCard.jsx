...
const AlbumCard = ({ album, size = 'md' }) => {
    ...
    const handleClick = () => {
        const { filterSongsByAlabum } = useApp();
        filterSongsByAlbum(album.id, album.title);
    };
    ...
    return (
        ...
        >
            ...
            <h3 className={`${textSizes[size]} font-medium text-center mt-2 text-gray-800 w-full truncate px-1`}>
                {album.title}
            </h3>
            <p className="text-xs text-gray-500 text-center w-full truncate px-1">
